...
use App\Models\ACL\User as ACLUsuario;
...
class PermissionController extends Controller
{
    public function __construct()
    {
      ...
      $form->addField('orden', 'input:number');
    ...
    public function index(Request $request)
    {
      ...
      return view('acl.permissions.index', compact('roles','modulos', 'grupos'));
    }
    public function UsuarioPermisos(Request $request, ACLUsuario $aclusuario)
    {
        if ($request->isMethod('post')) {
            $accion = $request->input('accion');
            ACLUsuarioGrupo::where('usuario_id', $aclusuario->id)->update(['eliminado' => 1])->get();
            if(!empty($accion)) {
    ...
    public function GrupoPermisos(Request $request, ACLGrupo $aclgrupo)
    {
        if ($request->isMethod('post')) {
          $accion = $request->input('accion');
         ACLGrupoPermiso::where('grupo_id', $aclgrupo->id)->update(['eliminado' => 1])->get();
            #->update(['eliminado' => 1]);
            if(!empty($accion)) {
