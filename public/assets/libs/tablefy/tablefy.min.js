var FuncionesUtiles = {
  ...
  function Tablefy (params) {
    ...
    _instance.privado = {
      ...
      init: function(x) {
          ...
          html_header += "<tr class=\"header\">";
            ...
            if(!FuncionesUtiles.empty(parametros.selectable)) {
              if(typeof parametros.countSelectable === 'undefined' || parametros.countSelectable > 1) {
                _columns_additionals['selectable'] = 1;
        ...
        html_footer += "</tr></tbody></table></div><div class=\"tablefy_pagination\"><ul><li class=\"prev_page disabled\">Anterior</li><li class=\"current_page\"></li><li class=\"next_page disabled\">Siguiente</li></ul></div><div class=\"tablefy_records\"></div><div class=\"tablefy_execute\"></div><div class=\"tablefy_console\"></div></td></tr></tfoot>";
      ...
      renderActions: function(ls, row) {
        ...
        var html = $('<ul>').addClass('list-group list-group-horizontal');
        if(!FuncionesUtiles.empty(ls)) {
          for(var index in ls) {
            if(ls.hasOwnProperty(index)) {
              if(typeof ls[index].link !== 'undefined') {
                html.append('<li class="list-group-item" tablefy-action="' + ls[index].uid + '"><a href="' + ls[index].link + '"><span class="bx bx-' + ls[index].icon + '"></span></a></li>');
              } else {
                ...
                html.append($('<li>').addClass('list-group-item').attr('tablefy-action', ls[index].uid).html(item));
              }
            }
          }
        }
          ...
          $.each(contenidx.result.items, function(k, v) {
            ...
            if(!FuncionesUtiles.empty(v.popy_father_id)) {
              ...
              if(!FuncionesUtiles.empty(v.popy_is_father)) {
                var html = "<tr data-padre=\"" + v.popy_id + "\"><td colspan=\"" + (count(parametros.headers) + _instance.privado.columnsAdditionals()) + "\"><div style=\"margin: -1px -2px;border-left: 1px solid #cab637;margin-left: 15px;padding-left: 15px;margin-right:0px;min-height: 8px;background: #fbec88;\"><table style=\"margin: -1px;\"><tbody></tbody></table></div></td></tr>";
  ...
  //      }
        ...
        tr.append($('<td>').addClass('tablefy-td-actions').html(_instance.privado.renderActions(parametros.actions, row)));
        return tr;
      },
      shake: function(xx) {
      ...
      refreshOptions: function() {
        ...
        elementosTablefy.footer.find("tfoot>tr>td>.tablefy_pagination .current_page").text('PÃ¡gina ' + _instance.current_page);
